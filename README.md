...
- 🚀 **Фриланс-режим** — отдельный тип откликов (vacancy/freelance): бюджет, конкурс, дедлайн, рейтинг звёздами. Загрузка ТЗ из PDF → парсинг → в промпт → более релевантный отклик
...
|-------------|------|-----------|
| [cv-backend](https://github.com/interactive-cv/cv-backend) | Python, FastAPI, PostgreSQL | REST API, AI-промпты (чат + генерация + freelance + правка CV), PDF-экспорт, трекинг, сессии, аналитика |
...
VPS + Docker Compose одной командой. Подробности — в README обоих репозиториев.

## Дорожная карта

- [x] Универсализация шаблона (env-параметризация)
- [x] Админ-панель с AI-генерацией откликов
- [x] Фриланс-режим (бюджет, конкурс, рейтинг, ТЗ из PDF)
- [x] Экспорт CV в PDF
